...
async function init() {
  ...
  // Doctor Routes
  app.post("/doctor/addWDR", (req, res) => doctor_addWDR(req, res));
  app.post("/doctor/getWDR", (req, res) => doctor_getWDR(req, res));
  ...
  app.post("/patient/getWDR", (req, res) => patient_getWDR(req, res));
  app.post("/patient/viewAllWDR", (req, res) => viewAllWDR(req, res));

  app.post("/patient/viewAllUsersWithAddAccess", (req, res) =>
    viewAllUsersWithAddAccess(req, res)
  );

  app.post("/patient/getGrantedUserForWDR", (req, res) =>
    getGrantedUserForWDR(req, res)
  );
...
// Doctor Routes
async function doctor_addWDR(req, res) {
  try {
    await query.initialize("doctor");
    const result = await query.doctor_addWDR(
      req.body.userEmail,
      req.body.adderEmail,
      req.body.wdrID,
      req.body.type,
    ...
    console.error(`error: doctor_addWDR: ${error}`);
  }
}

async function doctor_getWDR(req, res) {
  try {
    await query.initialize("doctor");
    const result = await query.doctor_getWDR(
      req.body.wdrID,
      req.body.userEmail
    );
    res.status(200).send(result);
    ...
    console.error(`error: doctor_getWDR: ${error}`);
  }
}
...
async function patient_getWDR(req, res) {
  try {
    await query.initialize("patient");
    const result = await query.patient_getWDR(
      req.body.wdrID,
      req.body.userEmail
    );
    res.status(200).send(result);
    ...
    console.error(`error: patient_getWDR: ${error}`);
  }
}
...
async function viewAllUsersWithAddAccess(req, res) {
  try {
    ...
    console.error(`error on getWDRRoute: ${error}`);
  }
}

async function getGrantedUserForWDR(req, res) {
  try {
    await query.initialize("patient");
    const result = await query.getGrantedUserForWDR(
      req.body.wdrID,
      req.body.userEmail
    );
    res.status(200).send(result);
...
async function viewAllWDR(req, res) {
  // wdrID: wdrID,
  // userEmail: userEmail
  try {
    console.log(req.body.userEmail);
    await query.initialize("patient");
    const result = await query.viewAllWDR(req.body.userEmail);
    res.status(200).send(result);
  } catch (error) {
    res.status(500).send("Internal server error");
    console.error(`error on getWDRRoute: ${error}`);
