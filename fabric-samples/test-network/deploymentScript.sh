...
checkCommitReadiness() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name wdr --version ${VERSION} --sequence ${VERSION} --output json --init-required >&log.txt
    res=$?
    set +x
    let rc=0
    for var in "$@"
...
setGlobals 1
peer lifecycle chaincode package wdr.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label wdr_${VERSION} >&log.txt
echo "===================== Chaincode packaged - peer0.org1 ===================== "

# Install chaincode on peer0.org1
echo "Installing chaincode -> peer0.org1"
setGlobals 1
peer lifecycle chaincode install wdr.tar.gz >&log.txt
...
cat log.txt
PACKAGE_ID=$(sed -n "/wdr_${VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}" log.txt)
echo "===================== Query installed successful on peer0.org1 on channel ===================== "

# Approve chaincode definition for org1
echo "Approve chaincode definition for org1..."
setGlobals 1
peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name wdr --version ${VERSION} --init-required --package-id ${PACKAGE_ID} --sequence ${VERSION} >&log.txt
cat log.txt
...
peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name wdr ${PEER_CONN_PARMS[@]} --version ${VERSION} --sequence ${VERSION} --init-required >&log.txt
...
setGlobals 1
peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name wdr >&log.txt
...
parsePeerConnectionParameters 1 2
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n wdr ${PEER_CONN_PARMS[@]} --isInit -c '{"function":"initLedger","Args":[]}' >&log.txt
